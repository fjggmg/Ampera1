...
public class KeySigEntropyPair {
    public KeySigEntropyPair(String sig, String entropy, List<String> inputs, String prefix, boolean p2sh) {
        this.sig = sig;
    ...
    static KeySigEntropyPair fromAmplet(HeadlessPrefixedAmplet amplet) {
        try {
            ...
            return new KeySigEntropyPair(sig, entropy, inputs, prefix, p2sh);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return null;
        }
