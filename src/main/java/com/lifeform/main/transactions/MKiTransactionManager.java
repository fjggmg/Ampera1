 ...
 */
public class MKiTransactionManager implements ITransManMKi {
    ...
    public MKiTransactionManager(IKi ki)
    {
        this.ki = ki;
    ...
    public boolean softVerifyTransaction(MKiTransaction t) {
        ...
        if(!EncryptionManager.verifySig(t.all(),t.signature,t.sender)) return false; //NOT YOUR TRANSACTION BROSKI
        ki.getMainLog().info("mark 6");
        if(!EncryptionManager.verifySig(t.preSigAll(),t.relaySignature,t.relayer)) return false; //WHO THE FUCK RELAYED THIS
