...
import com.lifeform.main.network.packets.adx.OrderCancelled;
import com.lifeform.main.network.packets.adx.OrderMatched;
import com.lifeform.main.network.packets.adx.OrderPacket;
import com.lifeform.main.network.packets.adx.OrderReduced;
    ...
    public OrderStatus placeOrder(boolean buy, BigInteger amount, BigInteger stopPrice, Pairs pair, boolean limitBuy) {
        ...
        if (buy) {
            ...
            for (Order o : orderBook.sells()) {
                ...
                List<Input> ourInputs = new ArrayList<>();
                ...
                inputs.addAll(toUs);
                List<Input> feeIn = ki.getTransMan().getInputsForAmountAndToken(ki.getAddMan().getMainAdd(), BigInteger.valueOf(1_000_00), Token.ORIGIN, true);
                if (feeIn == null) {
                    currentStatus = OrderStatus.BAD_UTXOS_US;
                    break;
                }
            ...
            if (amount.compareTo(BigInteger.ZERO) > 0 && limitBuy) {
                ...
                try {
                    ...
                    constants[1] = new DataElement(pair.onOffer().getName().getBytes("UTF-8"));
                    constants[2] = new DataElement(pair.accepting().getName().getBytes("UTF-8"));
            ...
            if (amount.compareTo(BigInteger.ZERO) > 0 && limitBuy) {
                ...
                try {
                    ...
                    constants[1] = new DataElement(pair.accepting().getName().getBytes("UTF-8"));
                    constants[2] = new DataElement(pair.onOffer().getName().getBytes("UTF-8"));
                    constants[3] = new DataElement(ki.getAddMan().getMainAdd().toByteArray());
        ...
        for (IAddress a : ki.getAddMan().getAll()) {
            ...
            if (a.encodeForChain().equals(o.address().encodeForChain())) {
                ...
                if (utxos != null && !utxos.isEmpty()) {
                    ...
                    List<Input> cIns = new ArrayList<>();
                    for (Output out : utxos) {
